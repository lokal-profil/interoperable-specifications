...
><span id="dv1"></span> **Rule DV-1:** A data vocabulary MUST be expressed in a single RDF Dataset.[^note1]
...
><span id="te1"></span> **Rule TE-1:** A terminology MUST be expressed in a single RDF Dataset.[^note1]
...
><span id="ap1"></span> **Rule AP-1:** An application profile MUST be expressed in a single RDF Dataset[^note2]
...
><span id="svg3"></span> **Rule SVG-3:** An element corresponding to an INSPEC entity MAY have an id on the form `id="d_EID"` where EID is the md5 sum[^note3] of the entity's URI.
